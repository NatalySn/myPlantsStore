...
export class AppComponent {
  ...
  cartClick() {
    ...
    this.totalPrice = Object.values(this.items).reduce(
      (a, b) => a + b.num * b.productPrice,
      0
    );
  }
// работа кнопок добавления и уменьшения товаров
  minusItem(index: any) {
  ...
  deleteItem(index: any) {
    ...
    if (((index = this.items.length - 1), (index = 0))) {
      ...
      this.totalPrice =
        this.totalPrice - deletelastItemPrice * deletelastItemNum;
    } else {
      this.totalAmount = this.totalAmount - deleteRegularItem[index].num;
      this.totalPrice =
        this.totalPrice -
        deleteRegularItem[index].num * deleteRegularItem[index].productPrice;
    }
